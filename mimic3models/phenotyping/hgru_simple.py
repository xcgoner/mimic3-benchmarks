...
class Network(Model):
    ...
    def __init__(self, dim, batch_norm, dropout, rec_dropout, task,
        ...
        for class_lv1 in label_struct.keys():
            y_lv1[class_lv1] = Dense(1, activation=final_activation)(L)
            L_lv2_dense = Dense(256, activation='relu')(L_lv1)
            if dropout > 0:
                L_lv2_dense = Dropout(dropout)(L_lv2_dense)
            L_lv2_dense = Dense(128, activation='relu')(L_lv2_dense)
            if dropout > 0:
                L_lv2_dense = Dropout(dropout)(L_lv2_dense)
            y_lv2[class_lv1] = {}
            for class_lv2 in label_struct[class_lv1]:
                y_lv2[class_lv1][class_lv2] = Dense(1, activation=final_activation)(L_lv2_dense)
